...
let utf_letter =
  [%sedlex.regexp?
    ( 0x0370 .. 0x03FF
    | 0x1D539 | 0x2102 | 0x210D | 0x2115 | 0x2119 | 0x211A | 0x211D | 0x2124 )]

let utf_op =
  [%sedlex.regexp? 0x2200 .. 0x22FF | 0x2190 .. 0x21FF | 0x00D7 | 0x00AC]
...
let rec token lexbuf =
  ...
  | Plus op ->
    ...
    let tok =
      match List.assoc_opt op' builtin_symbol with
      | Some op' -> op'
      | None -> OP op'
    in
    with_pos lexbuf tok
  ...
  | _ ->
    ...
    report_err
      ( Some (Location.of_lexbuf lexbuf),
        Printf.sprintf "Unrecognised character: %C." c )
...
and char lexbuf =
  ...
  | (letter | digit | symbol), '\'' ->
    with_pos lexbuf (CHAR (lexeme_char lexbuf))
  | _ ->
    let c = lexeme_char lexbuf in
    report_err
...
and string b lexbuf =
  ...
  | str ->
    ...
    string b lexbuf
  | eof ->
    report_err @@ (Some (Location.of_lexbuf lexbuf), "Unterminated string.")
  | _ ->
    let c = lexeme_char lexbuf in
    report_err
      ( Some (Location.of_lexbuf lexbuf),
