...
module Lexer : sig
  ...
  let skip (stream : t) ~am:(n : int) =
    if n < 0 then
      raise (Error.InternalError "Internal error - called Lexer.skip with a negative step.")
    else
      let rec aux t =
        if t <> 0 then
  ...
  let list_with_end (stream : t) (is_end : Token.token -> bool) (p : t -> 'a result) :
    ...
    let rec go l acc =
      ...
      | _ -> ( match p l with Error _ as e -> e | Ok v -> go l (v :: acc))
    in
    go stream []
  ...
  let consume_with (stream : t) (f : Token.token -> 'a option) (msg : string) : 'a result =
    let pos, next = advance stream in
    match f next with Some v -> Ok v | None -> make_err (Some pos, msg)
...
module Parser = struct
  ...
  let rec parse_args (l : Lexer.t) : located_pattern list Lexer.result =
    Lexer.list_with_end l
      (function
        | WILDCARD | LBRACK | LPAREN | IDENT _ | INT _ | FLOAT _ | STRING _ | CHAR _ | BOOL _
        | ATSIGN | UPPER_IDENT _ | UNIT ->
            false
        | _ -> true)
      parse_pattern
  ...
  and parse_pattern (l : Lexer.t) : located_pattern Lexer.result =
    ...
    | s, LBRACK ->
        let* r =
          ( (fun l ->
              let@ e =
                Lexer.consume_with_pos l RBRACK "Expected ']' to end empty list constructor."
              in
              (Location.combine s e, PVar "[]"))
          <|> fun l ->
            ...
            List.fold_right (fun n acc -> (loc, PBop (n, "::", acc))) (List.tl es) (List.hd es) )
            l
        in
        parse_pbop l r
    | s, LPAREN -> (
    ...
    | pos, tok ->
        Lexer.make_err (Some pos, Printf.sprintf "Expected pattern, but got '%s'." (Token.show tok))


  and parse_pbop (l : Lexer.t) ((s, _) as left : located_pattern) : located_pattern Lexer.result =
    ( (fun l ->
        let* o =
          Lexer.consume_with l (function OP o -> Some o | _ -> None) "Expected operator."
        in
        let@ ((e, _) as next) = parse_pattern l in
        (Location.combine s e, PBop (left, o, next)))
    <|> fun _ -> ok left )
      l
  ...
  and led (l : Lexer.t) (left : Ast.located_expr) (s : Location.t) (op : Token.token)
  ...
  and parse_binding (l : Lexer.t) ~(is_imp : bool) (s : Location.t) (om : operator_map) :
    ...
    let* _ = Lexer.consume l r_paren "Expected a ')' or '}' to end a type binding." in
    let* _ = Lexer.consume l ARROW "Expected an '->' after a type binding." in
    let@ ((e, _) as r) = parse_expr l 0 om in
    (Location.combine s e, Ast.Pi (Some (i, is_imp), l', r))
  ...
  and parse_let (l : Lexer.t) (om : operator_map) (s : Location.t) : Ast.located_expr Lexer.result =
    ...
    let* ty =
      ...
      | pos, tok ->
          Lexer.make_err
            (Some pos, Printf.sprintf "Unexpected token in let-expression: %s" (Token.show tok))
    in
  ...
  let rec parse_definition (l : Lexer.t) (om : operator_map) : Ast.located_definition Lexer.result =
    ...
    | pos, tok ->
        Lexer.make_err
          ( Some pos,
            Printf.sprintf "Expected 'dec' or 'def' keyword to begin a definition, but got '%s'."
              (Token.show tok) )
  ...
  and parse_def (l : Lexer.t) (om : operator_map) : Ast.located_definition Lexer.result =
    ...
    let* when_block =
      ...
      | pos, tok ->
          Lexer.make_err
            ( Some pos,
              Printf.sprintf "Expected ':' or ':=' after definition arguments, but got '%s'."
                (Token.show tok) )
    in
    ...
    let@ e, with_block =
      ( (fun l ->
          let* _ =
            Lexer.consume l WITH "Expected 'with' keyword to begin local function definitions."
          in
  ...
  and parse_record (l : Lexer.t) (om : operator_map) :
    ...
    let* _ =
      Lexer.consume l CONSTRUCTOR "Expected 'constructor' keyword to declare record constructor."
    in
  ...
  and parse_import_cond (l : Lexer.t) : (import_cond * Location.t) Lexer.result =
    let* cond_type =
      Lexer.consume_with l
        (function WITH -> Some true | WITHOUT -> Some false | _ -> None)
    ...
    let@ values, e =
      ...
      | _, LPAREN ->
          ...
          let@ p = Lexer.consume_with_pos l RPAREN "Expected a ')' to end the import condition." in
  ...
  let parse_program (l : Lexer.t) : Ast.program Lexer.result =
    ...
    let imports = List.filter_map (function Ast.TImport i -> Some i | _ -> None) prog in
    let tydecls = List.filter_map (function Ast.TTyDecl t -> Some t | _ -> None) prog in
    let body = List.filter_map (function Ast.TDef d -> Some d | _ -> None) prog in
