...
and pp_closure out ((env, tm) : closure) = 
  Format.fprintf out "[ %a ] ↦@,  %a" Format.(pp_print_list ~pp_sep:(fun out () -> fprintf out "; ") pp_val) env pp_tm tm
...
let rec quote (lvl : lvl) (v : val_) : tm =
  ...
  | VLam (p, cl) ->
      ...
      *)
      Lam (p, quote (lvl + 1) (cl $$ VRigid (p, lvl, [])))
  | VPi (n, l, cl) ->
...
let rename (loc : Location.t) (mv : mv) (p : partial) (v : val_) : tm result =
  ...
  and go p v : tm result =
    ...
    | VLam (a, cl) ->
        let@ b = go (extend_pn p) (cl $$ VRigid (a, p.delta, [])) in
        Lam (a, b)
    | VPi (n, l, cl) ->
        let* l = go p l in
        let@ r = go (extend_pn p) (cl $$ VRigid (n, p.delta, [])) in
        Pi (n, l, r)
    | VMatch (c, bs) ->
        let* c = go p c in
        let@ bs =
