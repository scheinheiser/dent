...
let rec pp_tm out (tm : tm) =
  ...
  | Let (p, ty, v, n) ->
    Format.fprintf out "@[<v 2>let %s : %a =@,%a@]@,in@,%a" p pp_tm ty pp_tm v
      pp_tm n
  | Lam (arg, body) ->
    Format.fprintf out "\\@[<v 2> %s. {@,%a@,}@]" arg pp_tm body
  | Match (c, bs) ->
...
and v_ap l r =
  ...
  | _ -> Error.internal "Internal Error - called `v_ap` on non-function."
...
and v_ap_bds env bds m =
  ...
  | _ -> Error.internal "Internal Error - mismatched env/bds list."
...
let force = function
  | VMeta (m, sp) as flex -> (
    ...
    | Solved v ->
      ...
      Log.dbg None (Format.asprintf "evaluated solution from mctx := %a@." pp_val v);
      v
    | Unsolved -> flex)
  | v -> v
  ...
  let rec go = function
    ...
    | Snoc (sp, s) -> (
      ...
      | VLocal (_, l, Snoc.Lin) -> (
        ...
        | false -> ok (gamma + 1, IM.add l gamma subs))
...
let define_tcon ~(id : string) ~(t : val_) (ctx : ctx) =
  {ctx with top = SM.add id {ty = t; def = TCon} ctx.top}

let define_rcon ~(id : string) ~(t : val_) ~(fields : (string * tm) list)
    (ctx : ctx) =
  {ctx with top = SM.add id {ty = t; def = RCon fields} ctx.top}
...
let uni_err loc pp_func ex got =
  err
    ( loc,
      Format.asprintf
        "Un@[<v 4>ification failed:@,Expected → %a@,Received → %a@,@]" pp_func
        ex pp_func got )
...
let rec unify (loc : Location.t) (ctx : ctx) (l : val_) (r : val_) : unit result
  ...
  let rec unify_sp ctx sp sp' =
    ...
    | Snoc (sp, s), Snoc (sp', s') ->
      ...
      unify loc ctx s s'
    | _ -> err (Some loc, "Rigid mismatch in unification.")
  in
  ...
  | VTop (i, sp), t | t, VTop (i, sp) -> (
    ...
    | None -> err (Some loc, Printf.sprintf "Undefined identifier - '%s'." i)
    | Some (Def b, _) ->
      let b = to_val Snoc.empty b |> v_ap_sp sp in
      unify loc ctx b t
    | Some (DCon _, t') | Some (TCon, t') -> unify loc ctx t' t
    | Some (RCon _, _) -> Error.todo ""
    | Some (Axiom, _) ->
      err (Some loc, Printf.sprintf "The function '%s' has no definition." i))
  | VMeta (mv, sp), VMeta (mv', sp') when mv = mv' -> unify_sp ctx sp sp'
  | VMeta (mv, sp), t | t, VMeta (mv, sp) -> solve loc ctx.lvl mv sp t
...
and infer (ctx : ctx) ((loc, e) : Ast.located_expr) : (tm * val_) result =
  ...
  | Ast.Hole ->
    ...
    add_hole (loc, (to_val ctx.env a, t));
    ok (a, t)
  | Ast.Const c ->
    let t =
      match c with
  ...
  (* | Ast.Var (AccessIdent is) -> ( *)
  (*   let base = List.hd is in *)
  (*   let r = *)
  (*     Snoc.find_mapi *)
  (*       (fun n (x, t) -> if x = base then Some (n, t) else None) *)
  (*       ctx.tys *)
  (*   in *)
  (*   match r with *)
  (*   | None -> *)
  (*     err (Some loc, Printf.sprintf "Undefined identifier - '%s'." i) *)
  (*   | Some (n, t) -> ok (Local (i, n), t)) *)
  | Ast.Lam (x, t) ->
  ...
  | Ast.RCons (cons, fs) -> (
    ...
    | None ->
      err (Some loc, Printf.sprintf "Undefined record constructor - '%s'." cons)
    | Some (RCon ex_fs, sig_) ->
      let rec get_ret l = function
        | VPi (n, _, cl) -> get_ret (l + 1) (cl $$ VLocal (n, l, Snoc.empty))
        | v -> v
      in
    ...
    | Some (_, _) ->
      err
        ( Some loc,
          Printf.sprintf "Expected a record constructor, but got '%s'." cons ))
      ...
      let rec check_assoc ?(bind = false) ctx acc as_ =
        ...
        | (n, t) :: as_ -> (
          ...
          | Some (t, _) when bind ->
            ...
            check_assoc ctx (Some (n, t) :: acc) as_
      ...
      | (loc, (n, Ast.Record (cons, sig_'', fs))) :: ts -> (
        ...
        | Some (sig_, _) -> (
          ...
          | Some fs ->
            ...
            let ctx =
              define_rcon ~id:cons ~t:(to_val Snoc.empty csig) ~fields:fs ctx
            in
