...
let rec pp_tm out (tm : tm) =
  ...
  | IMv (m, bds) ->
      ...
      Format.fprintf out "?meta%d (%s)" m (Snoc.map to_str bds |> Snoc.fold_left (fun acc v -> Printf.sprintf "%s %s" v acc) "")
...
let rec pp_val out (v : val_) =
  ...
  | VTop (i, sp) -> 
    Format.fprintf out "%s (%s)" i (pp_sp pp_val sp)
  ...
  | VRigid (i, _, sp) ->
      Format.fprintf out "%s (%s)" i (pp_sp pp_val sp)
  | VFlex (m, sp) ->
      Format.fprintf out "?meta%d (%s)" m (pp_sp pp_val sp)
...
let rec to_val (env : env) (tm : tm) : val_ =
  match tm with
  | Local (n, i) -> 
    Log.dbg None (Printf.sprintf "indexing %s with %d into env.\n" n i);
...
and v_ap l r =
  ...
  | _ -> 
    Log.dbg None (Format.asprintf "failed → %a@." pp_val l);
    raise (Error.InternalError "Internal Error - called `v_ap` on non-function.")


(* unroll a spine *)
and v_ap_sp spine v = match spine with Snoc.Lin -> v | Snoc.Snoc(spvs, s) -> v_ap (v_ap_sp spvs v) s
and v_meta m = match MC.find m !mctx with Solved v -> v | Unsolved -> VFlex (m, Snoc.empty)
...
and quote_sp (lvl : lvl) (v : tm) (sp : spine) : tm =
  match sp with Snoc.Lin -> v | Snoc.Snoc (sp, s) -> Ap (0, quote_sp lvl v sp, quote lvl s)
...
*)
type partial = { gamma : lvl; delta : lvl; subs : lvl MC.t }

let extend_pn (p : partial) : partial =
  { gamma = p.gamma + 1; delta = p.delta + 1; subs = MC.add p.gamma p.delta p.subs }
...
let empty_ctx () = { env = Snoc.empty; top = TC.empty; tys = Snoc.empty; records = []; lvl = 0; bds = Snoc.empty }
...
let define_var ~(id : string) ~(v : val_) ~(t : val_) (ctx : ctx) =
  { ctx with env = ctx.env @> v; tys = ctx.tys @> (id, t); lvl = ctx.lvl + 1; bds = ctx.bds @> D }
...
let rec unify (loc : Location.t) (ctx : ctx) (l : val_) (r : val_) : unit result =
  ...
  | VPi (n, l, cl), VPi (n', l', cl') ->
      ...
      unify loc (bump_lvl ctx) (cl $$ VRigid (n, ctx.lvl, Snoc.empty)) (cl' $$ VRigid (n', ctx.lvl, Snoc.empty))
  | VLam (p, cl), VLam (p', cl') ->
      unify loc (bump_lvl ctx) (cl $$ VRigid (p, ctx.lvl, Snoc.empty)) (cl' $$ VRigid (p', ctx.lvl, Snoc.empty))
  | VLam (p, cl), t | t, VLam (p, cl)->
      (*
       ...
       (fun x -> M x) N => M N
      *)
      unify loc (bump_lvl ctx) (cl $$ VRigid (p, ctx.lvl, Snoc.empty)) (v_ap t (VRigid (p, ctx.lvl, Snoc.empty)))
  | VMatch (c, bs), VMatch (c', bs') when List.length bs = List.length bs' ->
      (*TODO: unify patterns *)
      let* _ = unify loc ctx c c' in
      List.map2 (fun (_, _, b) (_, _, b') -> unify loc ctx b b') bs bs' |> combine_errors_unit
  | VRigid (_, ri, sp), VRigid (_, ri', sp') when ri = ri'->
    unify_sp ctx sp sp'
  | VTop (i, sp), VTop (i', sp') when i = i' ->
    unify_sp ctx sp sp'
  | VTop (i, sp), t | t, VTop (i, sp) -> (
    ...
    | Some (Def b, _) -> 
      ...
      unify loc ctx b t
    | Some (Axiom, _) ->
      make_err (Some loc, Printf.sprintf "The function '%s' has no implementation." i))
  | VFlex (mv, sp), VFlex (mv', sp') when mv = mv' ->
      unify_sp ctx sp sp'
...
let rec check (ctx : ctx) ((loc, e) : Ast.located_expr) (ex : val_) : tm result =
  ...
  | Ast.Lam (p, b), VPi (_, l, cl) ->
      ...
      let@ b = check (bind_var ~id:i ~t:l ctx) b (cl $$ VRigid (i, ctx.lvl, Snoc.empty)) in
      Lam (i, b)
  | Ast.Let (p, t, b, n), t' -> (
      let i, b = replace_pattern p b in
      match t with
...
and infer (ctx : ctx) ((loc, e) : Ast.located_expr) : (tm * val_) result =
  ...
  | Ast.Var i -> (
      ...
      | Some (_, t) ->
        let t = force t in
        Log.dbg None (Format.asprintf "inferred variable %s.@.t := %a@." i pp_val t);
        ok (Top i, t)
      | None -> (
          let r = Snoc.find_mapi (fun n (x, t) -> if x = i then Some (n, t) else None) ctx.tys in
          match r with
          | None -> make_err (Some loc, Printf.sprintf "Undefined identifier - '%s'." i)
  ...
  | Ast.RCons (cons, fs) -> (
      ...
      | Some (_, cons, overall_t, ex_fs) ->
          ...
          if g <> e then
            ...
            let c =
              let r =
                Snoc.find_mapi (fun n (x, t) -> if x = cons then Some (n, t) else None) ctx.tys
              in
              match r with
              | None ->
                  raise (Error.InternalError "Internal error - record constructor not in ctx.")
              | Some (n, _) -> Local (cons, n)
            in
...
let rec check_definition (ctx : ctx) (loc, (i, args, wb, b, locals)) :
  ...
  *)
  ...
  | None ->
      ...
      ok @@ (d :: locals, ctx)
  | Some (Axiom, t') ->
    Log.dbg None (Format.asprintf "un@[<v>ifying func types.@,Left: %a@,Right: %a@,@]" pp_val t' pp_val t);
      ...
      (d :: locals, ctx)
  | Some (Def _, _) ->
    make_err (Some loc, Printf.sprintf "The function '%s' has already been defined." i)
    ...
    let rec check_decls ctx acc ts =
      ...
      | (loc, (n, Ast.Record (cons, sig_'', fs))) :: ts -> (
          ...
          | Ok sig_ -> (
              ...
              | Error _ as e -> check_decls ctx (e :: acc) ts
              | Ok fs ->
                  let csig = List.fold_left (fun acc (_, n) -> Pi (fresh_bind_var (), n, acc)) sig_ fs in
