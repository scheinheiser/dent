...
let fmt_debug src msg =
  match src with
  | Some (f, func, l, s, e) ->
    Format.asprintf "@[<v 2>[\x1b[1;33mDEBUG\x1b[0m]: %s@,file: %s@,function: %s@,loc: ln %d, chars %d-%d@]" msg f func l s e
  | None ->
    Format.asprintf "@[<v 2>[\x1b[1;96mDEBUG\x1b[0m]:@,%s@]" msg

let fmt_info msg =
  Format.asprintf "[\x1b[1;94mI@[<v>NFO\x1b[0m]:@,%s@]" msg

let fmt_warn loc msg =
  match loc with
  | Some l -> Format.asprintf "[\x1b[1;33mW@[<v>ARN\x1b[0m] %a:@,%s@]" Location.pp_location l msg
  | None -> Format.asprintf "[\x1b[1;33mW@[<v>ARN\x1b[0m]:@,%s@]" msg

let fmt_error loc msg =
  match loc with
  | Some l -> Format.asprintf "[\x1b[1;91mE@[<v>RROR\x1b[0m] %a:@,%s@]" Location.pp_location l msg
  | None -> Format.asprintf "[\x1b[1;91mE@[<v>RROR\x1b[0m]:@,%s@]" msg
